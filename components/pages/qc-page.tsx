...
export function QCPage() {
  ...
  const pending = getRecordsByStage("weighment", "Verified");
  const history = getRecordsByStage("qc", "QC Done");
  ...
  const handleQC = (record: (typeof pending)[0]) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedRecord) {
      const qcReportNo = `QC-${String(history.length + 1).padStart(3, "0")}`;
      updateRecord(selectedRecord.id, {
        qcReportNo,
        sampleResult: formData.sampleResult,
        testDate: formData.testDate,
        approvalStatus: formData.approvalStatus,
        status: "QC Done" as const,
      });
      moveToStage(selectedRecord.id, "qc", "QC Done");
