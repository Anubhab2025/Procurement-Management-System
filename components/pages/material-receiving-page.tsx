...
export function MaterialReceivingPage() {
  ...
  const pending = getRecordsByStage("followup", "Follow-up Done");
  const history = getRecordsByStage("receiving", "Received");
  ...
  const handleReceiveMaterial = (record: (typeof pending)[0]) => {
    setSelectedRecord(record);
    setFormData({ vehicleNo: "", driverName: "" });
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedRecord) {
      const gateEntryNo = `GE-${String(history.length + 1).padStart(3, "0")}`;
      ...
      moveToStage(selectedRecord.id, "receiving", "Received");
  ...
  return (
    <div className="space-y-6 p-4 md:p-0">
      {/* Header */}
      <h2 className="text-2xl md:text-3xl font-bold text-gray-900">Material Receiving</h2>
                      ...
                      <tr key={record.id} className="hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3">
                          <Button
                            ...
                            className="bg-orange-600 hover:bg-orange-700 text-xs flex items-center gap-1"
                          >
