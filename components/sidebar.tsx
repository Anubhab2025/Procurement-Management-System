...
const iconMap: Record<string, React.ReactNode> = {
  ...
  RefreshCw: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>,
  Truck: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 17a2 2 0 11-4 0 2 2 0 014 0zm10 0a2 2 0 11-4 0 2 2 0 014 0zm-2-8h.01M9 9h.01M19 9H5a2 2 0 00-2 2v8a2 2 0 002 2h14a2 2 0 002-2v-8a2 2 0 00-2-2z" /></svg>,
  Scale: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v v4m0 0v4m0-4h4m-4 0H8" /></svg>,
  Beaker: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19.428 15.428a2 2 0 00-1.022-.547l-2.387-.477a6 6 0 00-3.86.517l-.318.158a6 6 0 01-3.86.517L6.05 15.21a2 2 0 00-1.022.547m7.5-.5v.01M19.5 7.5a2 2 0 11-4 0 2 2 0 014 0z" /></svg>,
    ...
    <>
      ...
      >
        ...
        <div className="p-5 flex items-center justify-between">
          <div className={`flex items-center gap-3 overflow-hidden transition-all ${isOpen ? "w-40" : "w-0"}`}>
            ...
            return (
              <div key={item.id} className="relative group">
                ...
                >
                  ...
                  <div className={`relative z-10 transition-all duration-200 ${isActive ? "scale-110" : "group-hover:scale-125"}`}>
                    {Icon}
                  </div>
                  <span
                    className={`
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-gradient-to-t from-slate-900 to-slate-800 border-t border-slate-700 shadow-2xl z-50">
          ...
          {menuItems.slice(0, 5).map((item) => {
            ...
            return (
              <button
                ...
                `}
              >
                <div className={`transition-all ${isActive ? "scale-125" : "group-hover:scale-110"}`}>
                  {Icon}
                </div>
                <span className="text-xs font-medium">{item.label.split(" ")[0]}</span>
                {isActive && <div className="w-1 h-1 bg-cyan-400 rounded-full" />}
              </button>
            )
          })}
          <button
      ...
      {/* Mobile Full Menu (when "More" is clicked) */}
      {isOpen && (
        <div className="md:hidden fixed inset-0 bg-black/50 z-50" onClick={onToggle}>
          <div
            className="absolute bottom-16 left-4 right-4 bg-slate-800 rounded-2xl shadow-2xl p-4 animate-in slide-in-from-bottom duration-300"
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isActive ? "bg-gradient-to-br from-blue-600 to-cyan-500 text-white" : "bg-slate-700 text-slate-300"}
                    `}
                  >
                    {Icon}
